...
void GraphItem::keyPressEvent(QKeyEvent *event)
{
    if (event->key() == Qt::Key_Z) {
        // 获取当前鼠标在场景中的位置
        // 注意：这里需要根据具体实现调整坐标转换
        QPointF scenePos = mapToScene(mapFromScene(QCursor::pos()));

        // 转换为时间点
        //pScene->menu_time = pScene->conv_Display2Point_x(scenePos.x());

        // 执行插入关键帧操作
        pScene->act_insertKeyFrame();
